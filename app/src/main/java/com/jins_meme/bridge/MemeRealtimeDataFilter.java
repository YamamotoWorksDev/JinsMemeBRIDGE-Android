...
public class MemeRealtimeDataFilter {
  ...
  public boolean isLeft() {
    return isCommand(COMMAND_LEFT);
  }

  public boolean isRight() {
    return isCommand(COMMAND_RIGHT);
  }

  public boolean isUp() {
    return isCommand(COMMAND_UP);
  }

  public boolean isDown() {
    return isCommand(COMMAND_DOWN);
  }

  public boolean isBlink() {
    return isCommand(COMMAND_BLINK);
  }
  ...
  public void update(MemeRealtimeData memeRealtimeData, int thresholdBlink, int thresholdUD,
      int thresholdLR) {
    update(memeRealtimeData, thresholdBlink, thresholdUD, thresholdUD, thresholdLR, thresholdLR);
  }

  public void update(MemeRealtimeData memeRealtimeData, int thresholdBlink, int thresholdUp,
      int thresholdDown, int thresholdLeft, int thresholdRight) {
    if (isWaiting()) {
      mLastCommand = COMMAND_NONE;
      return;
    }
